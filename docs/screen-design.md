...
- 길게 누름(1.5s+)은 모든 상태에서 **세션 취소 → IDLE** 로 예약한다.
- STT/파싱(①)이 붙기 전까지 PARSING 은 스킵 가능하다 (LISTENING → AIMING 직행).
...
- [ ] 세션 상태 머신 + 볼륨 트리거 구현 (⑤)
- [ ] 프레임 링 버퍼(전후 1초) + 백엔드 업로드 클라이언트 (⑤)
- [ ] IMU 기울기 센서 연동 (⑤)
- [ ] 온보딩·설정 화면 및 TalkBack 대응 (⑥)
- [ ] 편차 신호 → 햅틱·사운드 패턴 설계 (⑥)
- [ ] 결과 확인 화면 + MLLM 설명 낭독 (⑥, ④ 연동)
