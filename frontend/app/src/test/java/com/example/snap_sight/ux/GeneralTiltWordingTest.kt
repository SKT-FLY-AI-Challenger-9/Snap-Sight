...
class GeneralTiltWordingTest {
    ...
    fun `subject high with camera tilted down speaks tilt-toward wording`() {
        ...
        val actions = GuidancePolicy().feed(result(y = -0.35f), now = 0, phonePitch = 30f)
    ...
    fun `pitch below swap threshold keeps the move wording`() {
        val actions = GuidancePolicy().feed(result(y = -0.35f), now = 0, phonePitch = 5f)
        assertEquals(listOf(GuidanceDirection.UP.utterance), speech(actions))
    }
    ...
    fun `null pitch keeps existing behaviour untouched`() {
        val actions = GuidancePolicy().feed(result(y = -0.35f), now = 0, phonePitch = null)
        assertEquals(listOf(GuidanceDirection.UP.utterance), speech(actions))
    }
    ...
    fun `tilt wording repeats only after the repeat interval`() {
        ...
        policy.feed(result(y = -0.35f), now = 0, phonePitch = 30f)
        assertTrue(policy.feed(result(y = -0.35f), now = 500, phonePitch = 30f).isEmpty())
    ...
    fun `leveling the phone switches back to the move wording after the gap`() {
        ...
        assertEquals(listOf(GuidanceDirection.UP.utterance), speech(leveled))
    }
}
