...
def test_build_requirements_from_text_end_to_end(raw_text, expected):
    """실제 발화를 파서에 통과시킨 결과가 기대한 요구사항과 일치한다."""
...
def test_parse_failure_returns_no_requirements(monkeypatch):
    ...
    monkeypatch.setattr("backend.mllm.target_requirements.parse_target_spec", _raise)
